window.addEventListener("load", () => {
  ...
  // 转义正则元字符：搜索词含 . * + ? [ ] ( ) { } ^ $ | \ 时，
  // 直接 new RegExp(keyword) 会抛 SyntaxError 导致结果区不再渲染（搜索卡死）。
          ...
          if (isMatch) {
            if (firstOccur >= 0) {
              ...
              keywords.forEach(keyword => {
                ...
                // 用实际匹配到的文本做高亮，避免把用户输入原样拼进 HTML
                matchContent = matchContent.replace(regS, m => '<span class="search-keyword">' + m + "</span>");
                dataTitle = dataTitle.replace(regS, m => '<span class="search-keyword">' + m + "</span>");
              });
